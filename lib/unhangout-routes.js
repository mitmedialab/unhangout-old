...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/followup/event/:id", function(req, res) {
            ...
            //Need to make decision on the MINIMUM TIME 
            //For now not considering MINIMUM TIME
            ...
            _.each(history.event, function(elapsedTime, userId) {
                ...
                _.each(history.sessions, function(usersTimes, sessionId) {
                    // Check if the user attended this session for longer than MINIMUM_TIME
                    
                    if (usersTimes[user.id] ) {
                            
                        _.each(usersTimes, function(time, userId) {
                            if (userId !== user.id) {

                                otherUserIds[userId] = 1;
                            }
            ...
            // Now get the full user objects for each other user's id:
            var otherUsers = [];

            for (var userId in otherUserIds) {
                otherUsers.push(db.users.get(userId));
            }
            ...
            if(valid) {
                ...
                res.render("email_template.ejs", {
                    ...
                    var message = {
                        ...
                        "from_name": "srishti",
                        "to": [{
                                "email": "unhangout.developer@gmail.com",
