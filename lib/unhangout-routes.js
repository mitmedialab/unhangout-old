...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/event/:id", ensureAuthenticated, function(req, res) {
            ...
            if (event.get("connectedUsers").length < options.EVENT_USER_CAP) {
                return res.render('event.ejs', context);
            } else {
                context.title += " - Overflow";
                return res.render("overflow.ejs", context);
            }
        });
        ...
        if (process.env.NODE_ENV != "production") {
            ...
            app.get("/test/hangout/gadgetcontents", function(req, res) {
                ...
                var gadgetUrl = options.baseUrl + "/hangout/gadget.xml?sessionId=" + req.query.sessionId + "&mockHangoutUrl=" + encodeURIComponent(req.query.mockHangoutUrl) + "&mockUserIds=" + req.query.mockUserIds + "&isHoA=" + req.query.isHoA;
