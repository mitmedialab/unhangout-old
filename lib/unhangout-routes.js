...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/auth/google-alt", passport.authenticate('google-alt', {
                scope: [
                    'https://www.googleapis.com/auth/userinfo.profile',
                    'https://www.googleapis.com/auth/userinfo.email']
            }),
        ...
        app.get("/auth/google/callback-alt", passport.authenticate('google-alt'),
            function(req, res) {
                // if post-auth-path was set, send them to that path now that authentication
                // is complete.
                if("post-auth-path" in req.session) {
        ...
        app.post("/myevents/admin-login-invite/", ensureAuthenticated, utils.ensureAdmin, function(req, res) {
            ...
            if (!req.user.isAdminOf(event)) {
                return res.send(400, "Not an admin of the event");
            }

            var valid = (
                "adminInviterName" in req.body &&
                "eventTitle" in req.body &&
                "adminInviterEmail" in req.body &&
                "adminInviteeEmail" in req.body && 
                "eventID" in req.body
            )

            if(valid) {
                //Render the body of the email to send to invitee
                res.render("admin-login-invite.ejs", {
                   eventTitle: req.body.eventTitle,
                   adminInviterName: req.body.adminInviterName,
                   adminInviteeEmail: req.body.adminInviteeEmail,
                   adminInviterEmail: req.body.adminInviterEmail,
                   eventID: req.body.eventID
                }, function(err, html) {
                    if (err) {
                        logger.error("There is an error rendering email body", err);
                        return res.send(500, "Something went wrong while sending your message. \
                             Please try again or email us directly at unhangout@media.mit.edu");
                    }   
                    ...
                    var message = {
                        ...
                        "headers": {"Reply-To": req.user.get("emails")[0].value},
                        "important": false,
                        "track_opens": true,
                        "track_clicks": true,
                        "auto_text": true
                    ...
                    mandrill_client.messages.send({"message": message}, function(result) {
                        if(result[0].status == 'sent') {
                            return res.send(200, "Your invite has been sent successfully.");
                        } else {
                            return res.send(500, "Something went wrong while sending your invite. \
                             Please try again or email us directly at unhangout@media.mit.edu.");
                        }
                    });
                })
        ...
        app.get("/myevents/", ensureAuthenticated, utils.ensureAdmin,function(req, res) {
            showEventOverview(req, res, "myevents.ejs");
        });

        app.post("/myevents/", ensureAuthenticated, utils.ensureAdmin, function(req, res) {
            var eventID = req.body.eventId; 
            var event = db.events.get(req.body.eventId);
            ...
            if(req.body.admins) {
                for (var c = 0; c< req.body.admins.length; c++) {
                    var user = db.users.get(req.body.admins[c]);
                    if (!user) { 
                        return res.send(400, "Unrecognized user"); 
                    }
                    if(req.body.action == 'add-event-admin') {
                        event.addAdmin(user);
                    } else if (req.body.action == 'remove-event-admin') {
        ...
        function updateEvent(event, attrs, user) {
            ...
            if (attrs.dateAndTime && attrs.timeZoneValue) {
                ...
                var validTimezone = _.indexOf(zones, attrs.timeZoneValue) !== -1;
                if (date && validTimezone && date.isValid()) {
        ...
        app.get("/hangout/gadget.xml", function(req, res) {
            ...
            if (process.env.NODE_ENV !== "production") {
                ...
                context.mockAppData = "sessionId:" + (req.query.sessionId || "1") +
                    ":baseUrl:" + encodeURIComponent(req.query.baseUrl);
            ...
            app.get("/test/hangout/gadgetcontents", function(req, res) {
                ...
                var gadgetUrl = options.baseUrl +
                    "/hangout/gadget.xml?sessionId=" + req.query.sessionId +
                        ...
                        (req.query.baseUrl ? "&baseUrl=" + encodeURIComponent(req.query.baseUrl) : "");
...
function generateSessionHangoutGDParam(session, user, options) {
    var doubleEncodedBaseUrl = encodeURIComponent(encodeURIComponent(options.baseUrl));
    return "?gid="+options.UNHANGOUT_HANGOUT_APP_ID + "&gd=sessionId:" + session.id + ":baseUrl:" + doubleEncodedBaseUrl + ":sockKey:" + user.get("sock-key") + ":userId:" + user.id;
}

function generateTestSessionHangoutGDParam(session, user, options) {
    return "?sessionId=" + session.id + "&sockKey=" + user.get("sock-key") + "&userId=" + user.id + "&baseUrl=" + encodeURIComponent(options.baseUrl) + "&mockUserIds=" + user.id;
}
