...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        var hangoutGadgetRoute = function(req, res, context) {
            if (process.env.NODE_ENV !== "production") {
                context.mock = true;
        ...
        app.get("/hangout/gadget.xml", function(req, res) {
            ...
            hangoutGadgetRoute(req, res, context);
        });

        app.get("/hangout/local-gadget.xml", function(req, res) {
            var unhangoutBaseUrl = baseUrl("localhost", options.UNHANGOUT_LOCALHOST_PORT, options.UNHANGOUT_USE_SSL);
            var context = {
                unhangoutBaseUrl: unhangoutBaseUrl
            }
            hangoutGadgetRoute(req, res, context);
        });
