...
module.exports = {
    ...
    route: function(app, db, options) {
                ...
                googleapis.discover('youtube', 'v3').execute(function(err, client) {
                    client.youtube.channels.list({
                        ...
                        logger.analytics("create-hoa", {
                            ...
                            hasGooglePlusId: !!googlePlusId
                        })
                        // If the user doesn't have any youtube channel associated with G+,
                        // they will be prompted to connect one, and go through
                        // mobile phone authorization and all that jazz.
                        return res.redirect(302, 
                            "https://plus.google.com/hangouts/_/?hs=20&authuser=0&hso=0" +
                            "&gid=" + options.UNHANGOUT_HANGOUT_APP_ID + 
        ...
        app.post("/admin-request/", utils.rejectUnlessAuthenticated, _.bind(function(req, res) {
            var valid = (
        ...
        app.get("/admin/event/new", ensureAuthenticated, utils.ensurePerm("createEvents"), _.bind(function(req, res) {
            var context = {user:req.user, events:db.events, create:true};
            res.render('admin-event.ejs', context);
        }, this));

        app.post("/admin/event/new", ensureAuthenticated, utils.ensurePerm("createEvents"), _.bind(function(req, res) {
            ...
            var timeZoneValue = req.body.timeZoneValue;
            var splits = timeZoneValue.split(","); 
            var tzAbbr = splits[0]; 
            ...
            var event = new models.ServerEvent({title:req.body.title, shortName:req.body.shortName, description:req.body.description,
                    ...
                    dateAndTime: req.body.dateAndTime, timeZoneValue: req.body.timeZoneValue,
                    timeZoneAbbr: tzAbbr
                });
        ...
        app.post("/admin/event/:id", ensureAuthenticated, ensureEventAdmin, _.bind(function(req, res) {
            ...
            event.set("dateAndTime", req.body.dateAndTime);
            event.set("timeZoneValue", req.body.timeZoneValue);
            event.set("timeZoneAbbr", tzAbbr);
            event.save();
            ...
            event.logAnalytics({
                ...
                timeZoneValue: event.get("timeZoneValue"),
                timeZoneAbbr: event.get("timeZoneAbbr")
            });
