...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/browsertest/", function(req, res) {
            // Look for the test session to use.
            var session;
            var i = -1;
            var shortCode;
            do {
                i++;
                shortCode = "test" + i;
                session = db.permalinkSessions.findWhere({"shortCode": shortCode});
            } while (session &&
                     session.getNumConnectedAndJoiningParticipants() >= session.MAX_ATTENDEES);
            if (!session) {
                // Test session doesn't exist. Create it.
                session = new models.ServerSession({
                    isPermalinkSession: true,
                    shortCode: shortCode,
                    title: "Test Hangout " + i,
                    description: "Place to test your web browser and Google account"
                });
            ...
            context.hasGooglePlus = (
                context.isAuthenticated && req.user.get("link") &&
                req.user.get("link").indexOf("plus.google.com") != -1
            );
            return res.render("browsertest.ejs", context);
        });
        app.post("/browsertest/", utils.ensureAuthenticated, function(req, res) {
            var params = {};
            var keys = ["problem", "browser", "googleacct"];
            keys.forEach(function(key) {
                var val = req.body[key] && req.body[key].trim();
                if (val) {
                    params[key] = val;
                }
            });
            if (_.size(params) === 0) {
                return res.redirect("/browsertest/");
            } else {
                var msg = ["The following was reported by a user at " +
                            options.baseUrl + "/browsertest/."];
                msg.push("User: " + req.user.get("emails")[0].value);
                msg.push("User-agent: " + req.get("user-agent"));
                keys.forEach(function(key) {
                    if (params[key]) {
                        msg.push("");
                        msg.push("== reported " + key + " ==");
                        msg.push(params[key]);
                    }
                });
                msg.push("");
                msg.push("== detailed acct info from google ==");
                msg.push(JSON.stringify(req.user.get("_json"), null, 2));
                var mailOptions = {
                    from: options.UNHANGOUT_SERVER_EMAIL_ADDRESS,
                    to:   options.UNHANGOUT_MANAGERS,
                    subject: "Unhangout: Reported problem joining hangout",
                    text: msg.join("\n")
                };
                db.smtpTransport.sendMail(mailOptions, function(err, response) {
                    if (err) {
                        logger.error("Error sending email", err);
                        logger.error(JSON.stringify(mailOptions), null, 2);
                        return;
                    }
                    return res.redirect("/browsertest/");
                });
        ...
        app.get("/browsertest/login/", function(req, res) {
            // Specific login route to set post-auth-path.
            if (req.isAuthenticated()) {
                return res.redirect("/browsertest/");
            }
            req.session['post-auth-path'] = "/browsertest/"
