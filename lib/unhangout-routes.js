...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        function getFollowupContext(eventId, participantIndex) {   
            var event = db.events.get(eventId);
            if (!event) {
              return null;
            }
        ...
        app.get("/followup/event/:id/participant_:participantIndex", utils.ensureAuthenticated, utils.ensureSuperuser, function(req, res) {
            ...
            var context = getFollowupContext(
              req.params.id,
        ...
        app.post("/followup/event/:id/sent/", utils.ensureAuthenticated, utils.ensureSuperuser, function(req, res) {
            ...
            // Use async.each to handle rendering and sending emails asynchronously.
            // https://github.com/caolan/async#each

            async.each(userIndices, function(userIndex, callback) {
                var context = getFollowupContext(event.id, userIndex);
                if (!context) {
                  return callback();
                }
                ...
                res.render("post_event_followup_email.ejs", context, function(err, html) {
                    ...
                    if (err) {
                        logger.error("Error rendering email body", err);
                        return res.send(500, "Server Error for participant " + userIndex);
                    }

                    templateArr.push({html: html, user: user}); 
                }); 

            }, function (err) {
                if (err) {
                    // One of the iterations produced an error. All processing
                    // will now stop
                    logger.error("Email for participant" + userIndex + " could not be rendered", err);
                    res.send(500, "An error occured");
                } 
        
            });  

            if(templateArr.length != userIndices.length) {
                return res.send(500, "All email templates could not be successfully rendered");
            }
        
            async.each(templateArr, function(template, callback) {
                ...
                var message = {
                    "html": html,
                    "text": "Example text content", // XXX FIXME
                ...
                mandrill_client.messages.send({"message": message}, function(result) {
                    ...
                    if (err) {
                      logger.error("Mandrill error", err);
                      callback(err);
                    }
                });
                ...
                if (err) {
                    // One of the iterations produced an error. All processing
                    // will now stop
                    logger.error("An email could not be sent", err);
                    res.send(500, "An error occured");
                } else {
                    var msg = "Sent " + emailsSent + " out of " + userIndices.length + ".";
                    logger.info(msg);
                    res.send(200, msg);
                }   
            ...
            if(emailsSent == userIndices.length) {
                return res.send(500, "All emails were successfully delivered");
            }
        });
