...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        function generateUserData(attrs) {   

            var event = db.events.get(attrs.id);
            var history = event.get("history");

            var userData = [];

            _.each(history.event, function(elapsedTime, userId) {
        ...
        app.get("/followup/event/:id/participant_:participantIndex", utils.ensureAuthenticated, utils.ensureSuperuser, function(req, res) {
            ...
            //Call generate user data function to grab history of event participants
            var userData = generateUserData(req.params);   
            userData.unshift(null);

            if(userData) {
                res.render("email_preview_template.ejs", {
                    participantIndex: req.params.participantIndex,
                    userData: userData,
                    userDataCount: userData.length - 1,
                    event: db.events.get(req.params.id)
                });
            }
        });
        ...
        app.post("/followup/event/:id/sent/", utils.ensureAuthenticated, utils.ensureSuperuser, function(req, res) {
            ...
            var mandrill_client = new mandrill.Mandrill('YOUR MANDRILL API KEY');
            ...
            //Making use of async utility module for handling asynchronous javascript 
            //each(arr, iterator, callback) applies the function iterator to each item 
            //in arr, in parallel. The iterator is called with an item from the list, 
            //and a callback for when it has finished. If the iterator passes an error 
            //to its callback, the main callback (for the each function) is immediately 
            //called with the error. The documentation is here 
            //https://github.com/caolan/async#each

            async.each(userData, function(user, callback) {

                var participantIndex = userData.indexOf(user);

                res.render("email_template.ejs", {
                    event: event,
                    userData: userData,
                    ...
                    message = {
                        ...
                        "from_email": "noreply@media.mit.edu",
                        "from_name": "Unhangout Team",
                        "to": [{
                                "email": userData[participantIndex].user.get("emails")[0].value,
                                "name": userData[participantIndex].user.getShortDisplayName() 
                            }],
                        "headers": {
                            "Reply-To": "srishti@media.mit.edu"
                        },
                    ...
                    mandrill_client.messages.send({"message": message}, function(result) {
                        ...
                        if(emailsSent == userData.length) {
                            console.log("emails succcessfully sent" );
                ...
                //if any of the email processing produced an error, err would equal that error
                if(err) {
                    //One of the iterations produced an error.
                    //All processing will now stop
                    console.log("An email could not be sent");

                } else {
                    console.log("All emails have been sent succcessfully");
                }   
