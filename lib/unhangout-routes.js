    ...
    route: function(app, db, options) {
        ...
        app.get("/hangout/gadget.xml", function(req, res) {
            ...
            if (process.env.NODE_ENV != "production") {
                ...
                context.mockAppData = {
                    sessionId: req.query.sessionId || "1"
                }
            } else {
                context.mockHangoutUrl = false;
                context.mockAppData = false;
            }
        ...
        app.get("/facilitator/:sessionId/", ensureAuthenticated, function(req, res) {
            ...
            var event = db.events.find(function(e) {
                if (e.get("sessions").get(req.params.sessionId)) {
                    return true;
                }
            });
            var session = event.get("sessions").get(req.params.sessionId);
        ...
        if (process.env.NODE_ENV != "production") {
            ...
            app.get("/test/hangout/gadgetcontents", function(req, res) {
                ...
                var gadgetUrl = options.baseUrl + "/hangout/gadget.xml?sessionId=" + req.query.sessionId + "&mockHangoutUrl=" + encodeURIComponent(req.query.mockHangoutUrl);
