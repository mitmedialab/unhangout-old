...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/session/:sessionKey", ensureAuthenticated, function(req, res) {
            ...
            // Hangouts-on-air require different logic.
            ...
            session.addJoiningParticipant(req.user);
            return res.redirect(302, "/webrtc/" + req.params.sessionKey);
        });
                ...
                res.render("admin-login-invite.ejs", {
                   ...
                   eventID: req.body.eventID
                    ...
                    var message = {
                        ...
                        "from_email": "node@unhangout.media.mit.edu",
        ...
        app.get("/webrtc/:sessionKey/", utils.ensureAuthenticated, function(req, res) {
            if (!req.params.sessionKey || req.params.sessionKey === "undefined") {
                return res.send(404, "Not found");
            }
            var session, event;
            session = utils.getSession(req.params.sessionKey, db.events, db.permalinkSessions);
            if (!session) {
                return res.send(404, "Not found");
            }

            res.render("webrtc-session.ejs", {
                title: session.get("title") || "Breakout session",
                session: session,
                event: event,
                user: req.user
            });
