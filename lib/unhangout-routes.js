...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/followup/event/:id/participant_:participantCount", function(req, res) {

            var event = db.events.get(req.params.id);

            var eventTitle = event.get("title");
            var history = event.get("history");
            var participantCount = req.params.participantCount; 

            var allUserIds = [];
            allUserIds.push(null);
            ...
            _.each(history.event, function(elapsedTime, userId) {
                ...
                allUserIds.push({ user: user, users: users });

            });

            var allUserIdsCount = Object.keys(allUserIds).length - 1;

            res.render("email_preview_template.ejs", {
                eventTitle: eventTitle,
                participantCount: participantCount,
                allUserIds: allUserIds,
                allUserIdsCount: allUserIdsCount,
                event: event
            });
        });

        app.post("/followup/event/:id/sent/", function(req, res) {
            ...
            var allUserIdsCount = Object.keys(allUserIds).length;
            var mandrill_client = new mandrill.Mandrill('PLDkuaJHZNqb2iuhLSGzYQ');

            var message = {}; 
            var emailsSent = 0;

            //Execute a loop to send emails
            for (var participantCount = 0; participantCount < allUserIdsCount; participantCount++) {
                    ...
                    message = {
                        ...
                        "to": [{
                                "email": allUserIds[participantCount].user.get("emails")[0].value,
                                "name": allUserIds[participantCount].user.getShortDisplayName() 
                            }],
                        "headers": {
