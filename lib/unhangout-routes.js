...
module.exports = {
    ...
    route: function(app, db, options) {
        ...
        app.get("/blog", function(req, res) {
            ...
            var feedURL = 'http://unhangout.tumblr.com/rss'; 
            ...
            http.get(feedURL, function(res) {
                res.pipe(new feedparser({}))
                    ...
                    .on('end', function(){ 
                        serverRes.render('blog.ejs', {user: req.user, feeds: feeds
                    });
                });    
            });             
        });
        ...
        function updateEvent(event, attrs) {
            ...
            if ("organizer" in attrs) {
                update.organizer = attrs.organizer;
            }
            if ("description" in attrs) {
                update.description = utils.sanitize(attrs.description);
            }
            if ("welcomeMessage" in attrs) {
                update.welcomeMessage = utils.sanitize(attrs.welcomeMessage);
            }
            if (attrs.dateAndTime && attrs.dateAndTime != "null" && attrs.timeZoneValue) {
                try {
                    var date = moment.tz(attrs.dateAndTime, attrs.timeZoneValue);
                } catch (e) {
                    date = null;
                }
                if (date && date.isValid()) {
                    update.dateAndTime = attrs.dateAndTime;
                    update.timeZoneValue = attrs.timeZoneValue;
                } else {
                    error.dateAndTime = "Invalid date or timezone."
                }
            } else if (attrs.dateAndTime == "null") {
        ...
        app.post("/admin/event/new", ensureAuthenticated, utils.ensurePerm("createEvents"), _.bind(function(req, res) {
            ...
            for (var i = 0; i < required.length; i++) {
                if (!(required[i] in req.body)) {
                    return res.send(400, "Missing parameter `" + required[i] + "`.");
                }
            }

            var event = new models.ServerEvent();
            var err = updateEvent(event, req.body);
            if (_.size(err) > 0) {
