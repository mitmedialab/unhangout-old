...
exports.ServerUserList = client_models.UserList.extend({
    ...
    registerOrUpdate: function(accessToken, refreshToken, profile, done) {
        ...
        profile["google_json"] = _.clone(profile["_json"]);
        delete profile["_raw"];
...
exports.ServerSession = client_models.Session.extend({
    ...
    setHangoutUrl: function(url, user, hangoutId) {
        ...
        if(_.isNull(this.get("hangout-url"))) {
            ...
            // Treat hangouts.google.com urls as identical to plus.google.com ones.
            var normalizeGoog = function(hangoutUrl) {
              return hangoutUrl && hangoutUrl.replace("hangouts.google.com", "plus.google.com");
            }
            if (normalizeGoog(this.get("hangout-url")) !== normalizeGoog(url)) {
                this.logAnalytics({
    ...
    removeAssignedParticipant: function(user) {
        if (this.get("randomized")) {
            var assignees = this.get("assignedParticipants") || [];
            var without = _.without(assignees, user.id);
            if (without.length !== assignees.length) {
                this.set("assignedParticipants", without);
                this.save();
                return true;
            }
        }
