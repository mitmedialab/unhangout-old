...
exports.ServerUser = client_models.User.extend({
    ...
    changeNetworkList: function(eventId, atNameUser, toggle) {
      var clone = deepCopy(this.get("networkList") || {});
      if (!clone[eventId]) {
        clone[eventId] = [];
      }
      if (_.contains(clone[eventId], atNameUser.id)) {
        if (toggle) {
          logger.debug("REMOVE FROM NETWORK", atNameUser.id);
          clone[eventId] = _.without(clone[eventId], atNameUser.id);
          this.save({networkList: clone});
        }
      } else {
        clone[eventId].push(atNameUser.id);
        logger.debug("ADD TO NETWORK", atNameUser.id);
        this.save({networkList: clone});
      }
   }
});
