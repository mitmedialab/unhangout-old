...
exports.ServerSession = client_models.Session.extend({
    ...
    getState: function() {
        // There are four parameters that contribute to the state:
        // `hangout-start-time`, `hangout-url`, `hangout-pending`, and
        // `connectedParticipants`.  However, there are only two fully valid
        // and one limbo combination of these -- "started", "stopped", and
        // "pending".
        //
        // "started": {
        ...
        // }
        //
        // Any other combination of values -- a hangout-url zero participants,
        // a hangout-start-time with no hangout-url, etc. is an error
        // condition.
        ...
        // Valid states
        //
        if (!pending && hasUrl && hasStartTime && hasParticipants) {
            return "started";
        }
        if (!pending && !hasUrl && !hasStartTime && !hasParticipants) {
            return "stopped";
        }
        if (pending && !pendingStale && !hasUrl && !hasStartTime && !hasParticipants) {
            return "pending";
        }
    ...
    onRestart: function() {
        ...
        if (this.get("hangout-url")) {
            // If no one has re-joined this within timeout after restart, stop the session.
            setTimeout(function() {
                if (this.getNumConnectedParticipants() == 0) {
	...
	url: function() {
        if (this.event && this.get("isHoA")) {
            // Event hangouts-on-air: use hoa namespace.
	...
	startHangoutWithUser: function(user) {
		if(this.isHangoutPending()) {
            ...
            // Set a timeout which clears hangout-pending status to allow
            // others to start the hangout if this user did not.
            setTimeout(function() {
                if (_.isEqual(this.get("hangout-pending"), obj)) {
                    this.logAnalytics({action: "unfarmed start timeout"});
                    this.set("hangout-pending", null);
                }
            }.bind(this), this.HANGOUT_CREATION_TIMEOUT);
    ...
    logAnalytics: function(opts) {
        var tag = this.get("isPermalinkSession") ? "permalinks" : this.get("isHoA") ? "hoa" : "sessions";
    ...
    toClientJSONForUser: function(user) {
        // Generate JSON appropriate for the permissions that the given user has.
