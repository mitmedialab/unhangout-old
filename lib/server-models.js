...
exports.ServerUser = client_models.User.extend({
    ...
    changeNetworkList: function(roomId, atNameUser, toggle) {
      var networkListClone = _.clone(this.get("networkList") || {});
      if (!networkListClone[roomId]) {
        networkListClone[roomId] = [];
      }
      var roomList = networkListClone[roomId];

      if (_.contains(roomList, atNameUser.id)) {
        if (toggle) {
          logger.debug("REMOVE FROM NETWORK");
          roomList = _.without(roomList, atNameUser.id);
        }
      } else {
        roomList.push(atNameUser.id);
        logger.debug("ADD TO NETWORK");
      }
      networkListClone[roomId] = roomList;
      this.save({networkList: networkListClone});
   }
});
