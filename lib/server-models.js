...
var heartbeats = 0; // Andrew
var instances = 0; // Andrew
var users = 0; // Andrew
...
exports.ServerEvent = client_models.Event.extend({
	...
	userConnected: function(user) {
		...
		// Andrew logic for counting users -- TODO: make this work for all users array
		_.contains(this.get("connectedUsers"), this.id) ? users = users : users++;
...
exports.ServerSession = client_models.Session.extend({
	...
	defaults: function() {
		return _.extend(client_models.Session.prototype.defaults(), {
			...
			"total-instances":null,  // new and unused
			"total-seconds":null, // new and unused
			"user-seconds":null // new and unused
		});
	},
		...
		this.on("hangout-started", _.bind(function() {
			...
			this.heartbeatInterval = setInterval(_.bind(function() {
				...
				if(new Date().getTime() - this.get("last-heartbeat") > 8000) {
					...
					++heartbeats; // Andrew
					this.set("total-seconds", heartbeats * 5); // Andrew
					this.set("user-seconds", heartbeats * users * 5); // Andrew
				}
			}, this), 6000);
		}, this));
		...
		if(this.get("hangoutConnected")) {
			...
			if(this.get("isPermalinkSession")) {
				...
				this.on("change:collection", function() {
					++instances;
					this.trigger("hangout-started");
					this.set("total-instances", instances);
				}, this);
			}
		}
	},
