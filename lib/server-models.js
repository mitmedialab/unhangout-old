...
exports.ServerSession = client_models.Session.extend({
    ...
    JOINING_EXPIRATION_TIMEOUT: 15 * 1000,
    ...
    initialize: function(args) {
        ...
        // Hooks for auto-expiring joining participants, and broadcasting them to events
        this.on("add-joining-participant", function(session, user) {
            this.save();
            this.scheduleJoiningExpiration(user);
        }.bind(this));
        this.on("remove-joining-participant", function(session, user) {
            this.save();
            this.clearJoiningExpiration(user);
        }.bind(this));
    },
    scheduleJoiningExpiration: function(user) {
        ...
        this._broadcastJoiningParticipants();
    },
    clearJoiningExpiration: function(user) {
        if (this.joiningTimeouts[user.id]) {
            clearTimeout(this.joiningTimeouts[user.id]);
    ...
    _broadcastJoiningParticipants: function() {
        if (this.collection && this.collection.event) {
            var event = this.collection.event;
            event.trigger("broadcast", event, "joining-participants", {
                id: this.id, participants: this.get("joiningParticipants")
            });
        }
    },
    getState: function() {
