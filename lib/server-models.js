...
exports.ServerEvent = client_models.Event.extend({
	...
	sendChatFromUser: function(message, user) {
		var msg = new exports.ServerChatMessage({user:user, text:message});
		this.get("recentMessages").push(msg);
		this.broadcast("chat", msg.toJSON());
	...
	openSessions: function() {
		client_models.Event.prototype.openSessions.call(this);

		this.broadcast("open-sessions", {});

		// when sessions open
		if(this.get("fake-session-activity")) {

			// start all sessions if they need starting.
			this.get("sessions").each(function(session) {
				if(!session.get("hangout-connected")) {
					session.set("hangoutConnected", true);
					session.set("hangout-start-time", new Date().getTime());
					session.setConnectedParticipantIds([exports.server.users.at(0).id]);
				}
			}, this);
			...
			var id = setInterval(_.bind(function() {
				this.get("sessions").each(function(session) {
					...
					exports.logger.debug("numParticipants: " + numParticipants);

					if(rand > 0.5 && (numParticipants < 10)) {
						var existingParticipants = session.get("connectedParticipantIds");
						existingParticipants.push(exports.server.users.at(0).id);
	...
	closeSessions: function() {
		client_models.Event.prototype.closeSessions.call(this);

		this.broadcast("close-sessions", {});
		clearInterval(this.get("fake-session-activity-id"));
		this.set("fake-session-activity-id", null);
	},
