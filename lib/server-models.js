...
exports.ServerUser = client_models.User.extend({
    ...
    toClientJSON: function() {
        var props = {
            ...
            sessionAssignments: this.get("sessionAssignments")
        };
        if (this.isSuperuser()) {
   ...
   /**
    * Set a denormalization of the value of Session.assignedParticipants on the
    * user object, for convenience of letting a user know which session they
    * should subscribe to.
    */
   setSessionAssignment: function(eventId, sessionId) {
        var assign = deepCopy(this.get("sessionAssignments") || {});
        if(!assign[eventId] || sessionId == null) {
            delete assign[eventId];
        } 
        if(sessionId !== null) {
            assign[eventId] = sessionId;
        }
        this.set("sessionAssignments", assign);
        this.save();
   },
});
...
exports.ServerEvent = client_models.Event.extend({
    ...
    getRandomizedSessions: function() {
        return this.get("sessions").filter(function(sess) {
            return sess.get("randomized");
        });
    },
...
exports.ServerSession = client_models.Session.extend({
    ...
    addAssignedParticipant: function(userId) {
