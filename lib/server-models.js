...
exports.ServerUserList = client_models.UserList.extend({
    ...
    registerOrUpdate: function(accessToken, refreshToken, profile, done) {
        //TODO: Add analytics logging here when merging with that branch.
        logger.debug("login", profile.id);
        var user = this.get(profile.id);
        if (!user) {
            logger.debug("create new user", profile.id);
            user = new exports.ServerUser();
        } else {
            logger.debug("update existing user", profile.id);
        }
...
exports.ServerSession = client_models.Session.extend({
	...
	isHangoutPending: function() {
		...
		if(_.isNull(this.get("hangout-pending"))) {
            ...
            // it's been more than 15 seconds, give up and let this person be
            // the new designee. If someone DOES actually complete the process
            // after this, then we're sort of in trouble because they'll be
            // floating, and their app will attempt to set the url.
	...
	setHangoutUrl: function(url) {
		if(_.isNull(this.get("hangout-url"))) {
            ...
            if (this.get("hangout-url") != url) {
                logger.debug("Attempt to double-set hangout url failed - existing: " + this.get("hangout-url") + " requested: " + url);
                return false;
            }
		}
        return true;
