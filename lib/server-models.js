...
exports.ServerSession = client_models.Session.extend({
	...
	defaults: function() {
		return _.extend(client_models.Session.prototype.defaults(), {
			...
			"total-instances":0,  // new and unused
			"total-seconds":0, // new and unused
			"user-seconds":0 // new and unused
		});
	},
	...
	heartbeat: function() {
		var timeDiff = new Date().getTime() - this.get("last-heartbeat"); // Andrew
		this.set("last-heartbeat", new Date().getTime());
		this.set("total-seconds", this.get("total-seconds") + timeDiff); // Andrew
		this.set("user-seconds", this.get("total-seconds") * this.get("connectedUsers").length); // Andrew
	},
