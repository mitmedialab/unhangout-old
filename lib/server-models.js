...
exports.ServerHoASession = exports.ServerSession.extend({
    ...
    getState: function() {
        var state = exports.ServerSession.prototype.getState.call(this);
        // HoA's keep their URL always -- we don't try to dynamically fetch
        // alternate hangouts for them. If you need a new hangout, create a new
        // hoa session.
        if (state == "stale url") {
            state = "stopped";
        }
    ...
    stopWithDelay: function(requestTime) {
        // HoA's always stop immediately; but the URL is preserved.
        return this.onHangoutStopped();
    },
    onHangoutStopped: function() {
        // preserve the URL.  This is a little hacky, but seems cleaner than
        // re-implementing the rest of the logic of onHangoutStopped or
        // branching within the parent's implementation.
        var url = this.get("hangout-url");
        // Stop the hangout, including removing the hangout-url.
        exports.ServerSession.prototype.onHangoutStopped.call(this);
        // re-add hangout-url
        this.set("hangout-url", url);
        this.save();
    }
