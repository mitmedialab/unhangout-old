...
function generateSessionHangoutGDParam(session, options) {
	...
	return "?gid="+options.HANGOUT_APP_ID + 
		...
		session.get("session-key") +':'+ encodeURIComponent(session.get('title')) +':'+ encodeURIComponent(session.get("isPermalinkSession") ? session.get('description') : "none") +':'+
		session.get('shortCode') + ':' + session.get("isPermalinkSession") + ':' + encodeURIComponent(session.collection.event ? session.collection.event.get("title") : "none");
}