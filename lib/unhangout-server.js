...
exports.UnhangoutServer.prototype = {
	...
	init: function(options) {
		...
		if(this.options["mock-auth-admin"]) {
			// if true, use the admin user for mock instead of the non-admin. Useful for testing admin functionality.
			passportMock.prototype.mockUser = passportMock.prototype.mockAdminUser;
		}
	...
	start: function() {
		...
		if(this.options["mock-auth"]) {
			this.express.use(passportMock.initialize(new models.ServerUser(passportMock.mockUser)));
		...
		this.express.post("/admin/event/new", ensureAuthenticated, ensureAdmin, _.bind(function(req, res) {
			...
			console.log("created new event: " + JSON.stringify(event));
...
function ensureAdmin(req, res, next) {
	...
	// var testUser = new models.ServerUser(req.user);
	console.log("in admin check: " + JSON.stringify(req.user));
	// console.log("in admin check (funct): " + testUser.isAdmin());
	console.log("backup check: " + req.user.admin);
