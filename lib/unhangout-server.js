...
exports.UnhangoutServer.prototype = {
	...
	start: function() {
		...
		passport.serializeUser(_.bind(function(user, done) {
			// We store the google profile object as a string, since it seems to be more than
			// one level deep. We'll store our own additions to it as separate keys
			
			// initialize the user based on the google data.
			
			// this is the crux of my issues, in some ways

			// REMEMBER REDIS IS RAM
			// 	so
			// 1) check and see if user exists in database? that's a fetch?
			// 2) if it does, update it.
			// 3) if it doesn't, make a new one.
			...
			this.redis.hset("user:" + user.id, "google-profile", JSON.stringify(user), _.bind(function(err, res) {
				if(err) {done(err, user);}
				else {
					
					// This is mostly just for testing purposes. Could drop this without any negative
					// effects.
					this.redis.hincrby("user:" + user.id, "serialization-count", 1, function(err, res) {
						if(err) {done(true, user);}
						else {
							done(err, user);
						}
					});
		...
		passport.deserializeUser(_.bind(function(obj, done) {
			this.redis.hgetall("user:" + obj.id, _.bind(function(err, res) {
				if(err) {done(err, obj);}
				else {
					// this little dance is to extract the google part of the profile (which is a string)
					// and then merge it in with our additions to the profile.
					var userAttrs = JSON.parse(res["google-profile"]);
					delete res["google-profile"];
					_.extend(userAttrs, res);
					
					var user = new models.User(userAttrs);
					
					logger.debug("deserialized user: " + JSON.stringify(user.toJSON()));
					done(null, user);
				}
			}, this));
		}, this));
		...
		this.express.use(express.session({ secret: this.options.SESSION_SECRET}));
