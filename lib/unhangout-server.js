...
exports.UnhangoutServer.prototype = {
	...
	init: function(options) {
		...
		this.options = _.defaults(options, {"level":"debug", "transport":"console", "HOST":"localhost", "PORT":7777, "REDIS_HOST":"localhost", "REDIS_PORT":6379});
		var transports = [];
	...
	start: function() {
		...
		passport.deserializeUser(function(obj, done) {
			logger.debug("deserialize user: " + JSON.stringify(obj));
			done(null, obj);
		});
		...
		this.express.use(express.session({ secret: this.options.SESSION_SECRET}));          //TODO load the secret from env_vars
