		    ...
		    conn.on('data', _.bind(function(string) {
				...
				switch(message.type) {
					...
					case "blur":
						...
						var blur = new models.ServerUser({user:user, id:message.args.id});

						try {
							event.broadcast("blur", blur.toJSON());
							user.writeAck("blur");
						} catch (e) {
							user.writeErr("blur");
						}
					...
					case "focus":
						...
						var focus = new models.ServerUser({user:user, id:message.args.id});

						try {
							event.broadcast("focus", focus.toJSON());
							user.writeAck("focus");
						} catch (e) {
							user.writeErr("focus");
						}
