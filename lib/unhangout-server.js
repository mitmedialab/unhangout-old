		    ...
		    conn.on('data', _.bind(function(string) {
				...
				switch(message.type) {
					...
					case "create-session":
						...
						if("name" in message.args && "description" in message.args) {
							// make the new session!
							var newSession = new models.ServerSession({"name":message.args.name, "description":message.args.description});
							newSession.save();
