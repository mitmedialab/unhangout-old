...
exports.UnhangoutServer.prototype = {
    ...
    start: function() {
        ...
        // If specified, register an alternate callback URL.  This is used to
        // allow admins to log directly in to a backend behind a load balancing
        // proxy. Both ALT_AUTH_CALLBACK_DOMAIN and the frontend domain must be
        // valid OAuth redirects in the google configuration.
        if (this.options.ALT_AUTH_CALLBACK_DOMAIN) {
            passport.use('google-alt', new GoogleStrategy({
              clientID: this.options.UNHANGOUT_GOOGLE_CLIENT_ID,
              clientSecret: this.options.UNHANGOUT_GOOGLE_CLIENT_SECRET,
              callbackURL: this.options.ALT_AUTH_CALLBACK_DOMAIN + "/auth/google/callback-alt"
            }, _.bind(this.db.users.registerOrUpdate, this.db.users)));
        }
        ...
        // Allows auth in development environment without access to the Google
        // APIs. Set DEV_AUTH in conf.json to the Google user ID of the user
        // to authorize as. Note that the user must already exist in the
        // database for this to work!
        if (this.options.DEV_AUTH) {
            function devAuth(req, res, next) {
                req.user = this.db.users.get(this.options.DEV_AUTH);
                next();
            };
            this.app.use(_.bind(devAuth, this));
        }
