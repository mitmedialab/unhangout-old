	...
	start: function() {
		...
		this.express.post("/session/hangout/:id", _.bind(function(req, res) {
			...
			switch(req.body.type) {
				...
				// disappearing.
				case "heartbeat":
					logger.debug("session:" + session.id + ":heartbeat\tfrom:" + req.body.from + "\tparticipants: " + JSON.stringify(req.body.participants) + "\turl: " + req.body.url);
					session.heartbeat(req.body.participants, req.body.url + generateSessionHangoutGDParam(session, this.options));
					break;
				default: 
