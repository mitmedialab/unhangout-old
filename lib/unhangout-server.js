...
exports.UnhangoutServer.prototype = {
	...
	init: function(options) {
		...
		this.options = _.defaults(options, {"level":"debug", "transport":"console", "HOST":"localhost", "PORT":7777, "REDIS_HOST":"localhost", "REDIS_PORT":6379, "SESSION_SECRET":"fake secret"});
		var transports = [];
		...
		passport.deserializeUser(_.bind(function(obj, done) {
			this.redis.hgetall("user:" + obj.id, _.bind(function(err, res) {
				if(err) {done(err, obj);}
					...
					var user = JSON.parse(res["google-profile"]);
