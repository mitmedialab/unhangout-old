...
exports.UnhangoutServer.prototype = {
	...
	start: function() {
		...
		passport.use(new GoogleStrategy({
			...
			// all of them to see if any of them are a superuser email.
			// if any of them are, mark this user as a superuser.
			_.each(_.pluck(newUser.get("emails"), "value"), _.bind(function(email) {
				if(this.options.SUPERUSER_EMAILS.indexOf(email) > -1) {
					logger.info("Detected login from blessed email account("+this.options.SUPERUSER_EMAILS+"), granting superuser rights.");
					newUser.set("superuser", true);
				}
			}, this));
            ...
            logger.analytics("users", {action: "create", user: newUser, totalUsers: this.db.users.length});
