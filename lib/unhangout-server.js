		    ...
		    conn.on('data', _.bind(function(string) {
				...
				switch(message.type) {
					...
					case "create-session":
						...
						if("name" in msg.args && "description" in msg.args) {
							// make the new session!
							var newSession = new models.ServerSession({"name":msg.args.name, "description":msg.args.description});
							newSession.save();
					...
					case "unattend":
						...
						console.log(err);
						if(err instanceof Error) {
							console.log("writing unattend error");
