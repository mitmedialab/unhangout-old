...
exports.UnhangoutServer.prototype = {
		...
		this.redis.once("ready", _.bind(function(err) {
			...
			this.redis.select(this.options.REDIS_DB, _.bind(function(err, res) {
				...
				if(this.options['mock-auth']) {
					this.users.add(new models.ServerUser(passportMock.getMockUser(this.options['mock-auth-admin'])));
				} 
	...
	start: function() {
		...
		if(this.options["mock-auth"]) {
			this.express.use(passportMock.initialize(new models.ServerUser(passportMock.getMockUser(this.options['mock-auth-admin']))));
		...
		this.express.get("/admin/event/new", ensureAuthenticated, ensureAdmin, _.bind(function(req, res) {
			console.log("IN ADMIN EVENT NEW");
...
function ensureAdmin(req, res, next) {
	...
	// console.log(".admin: " + req.user.admin + "; isAdmin() " + req.user.isAdmin());
	console.log(JSON.stringify(req.user.attributes));
	console.log(JSON.stringify(req.user.attributes.admin));
	console.log(JSON.stringify(req.user.attributes.get("admin")));
	console.log("attributes" in req.user);

	if("attributes" in req.user) {
		console.log("checking");
		if(req.user.attributes.get("admin")) {
			console.log("found");
			next();
