...
exports.UnhangoutServer.prototype = {
	...
	init: function(options) {
		...
		this.events.add(new models.Event({id:0, title:"Scratch Pedagodgy Unhangout", organizer: "MIT Media Lab & ConnectedLearning.tv",
										...
										start: new Date().getTime(), end: new Date().getTime()+60*60*2}));
		...
		this.events.add(new models.Event({id:1, title:"Open Source Learning Unhangout", organizer: "MIT Media Lab & ConnectedLearning.tv",
										...
										start: new Date().getTime()+60*60*24*4, end: new Date().getTime()+60*60*24*4 + 60*60*2}));
	...
	start: function() {
		...
		this.sockjs.on('connection', function(conn) {
		    ...
		    conn.on('data', function(message) {
		        logger.info('message ' + conn + ":" + message);
		    });
		});
		...
		this.express.get("/", _.bind(function(req, res) {
			res.render('index.ejs', {user:req.user, events:this.events});
		}, this));
		
		this.express.get("/event/:id", _.bind(function(req, res) {
			...
			res.render('event.ejs', {user:req.user, event:e});
		}, this));
