					...
					var farmedSesssionURL = farming.getNextHangoutUrl(_.bind(function(err, url) {
						if(err || url==null) {
							...
							if(session.isHangoutPending()) {
								...
								res.redirect("https://plus.google.com/hangouts/_" + generateSessionHangoutGDParam(session, this.options));
							}
						} else {
							...
							if(session.getHangoutUrl()) {
								...
								session.set("hangout-url", url + generateSessionHangoutGDParam(session, this.options));
							}
			...
			switch(req.body.type) {
				...
				case "loaded":
					if(session && "url" in req.body) {
						...
						session.setHangoutUrl(url + generateSessionHangoutGDParam(session, this.options));
						res.status(200);
						res.send();
					} else {
...
function generateSessionHangoutGDParam(session, options) {
	return "?gid="+options.HANGOUT_APP_ID + 
		...
		session.get("session-key") +':'+ session.get('title') +':'+ session.get('description') +':'+
