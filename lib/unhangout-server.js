...
exports.UnhangoutServer.prototype = {
	...
	start: function() {
		...
		if(this.options.USE_SSL) {
			try {
				var privateKey = fs.readFileSync(this.options.PRIVATE_KEY);
	    		var cert = fs.readFileSync(this.options.CERTIFICATE);				
			} catch (e) {
			...
			logger.log("info", "HTTPS server not started, because USE_SSL=false");
			this.https = null;
		}
