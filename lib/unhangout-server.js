		    ...
		    conn.on('data', _.bind(function(string) {
				...
				switch(message.type) {
					...
					case "attend":
						...
						var session = getSession(message, user, event, "attend");
					...
					case "unattend":
						...
						var session = getSession(message, user, event, "unattend");
						if(session instanceof Error) return;
					...
					case "start":
						...
						var session = getSession(message, user, event, "start");
						if(session instanceof Error) return;
					...
					case "embed":
						...
						var session = getSession(message, user, event, "embed");
						if(session instanceof Error) return;
						
						if("ytId" in message.args) {
							session.setEmbed(message.args.ytId);
							user.writeAck("embed");
							// session.save();
						} else {
							user.writeErr("embed");
...
function getSession(message, user, event, type) {
