...
exports.UnhangoutServer.prototype = {
		...
		this.redis.once("ready", _.bind(function(err) {
			...
			this.redis.select(this.options.REDIS_DB, _.bind(function(err, res) {
				...
				if(this.options['mock-auth']) {
					this.users.add(passportMock.getMockUser());
				} 
	...
	start: function() {
		...
		if(this.options["mock-auth"]) {
			this.express.use(passportMock.initialize(passportMock.baseMockUser));
			...
			switch(req.body.type) {
				...
				case "participants":
					...
					if(session && "participants" in req.body) {
						...
						var participantIds = [];
						if(Object.keys(req.body.participants).length>0) {
							participantIds = _.map(req.body.participants, function(participant) {
