		...
		this.express.post("/session/hangout/:id", _.bind(function(req, res) {
			...
			switch(req.body.type) {
				...
				case "participants":
					...
					logger.info("setting participants on "+ session.id +": " + JSON.stringify(participantIds));

					session.setConnectedParticipantIds(participantIds);
					break;
				case "heartbeat":
					// TODO add heartbeat behavior to make sure the hangout is still online.
					break;
				default: 
