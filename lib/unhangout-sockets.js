...
_.extend(UnhangoutSocketManager.prototype, events.EventEmitter.prototype, {
    init: function(sockjs) {
        ...
        mgr.on("group-user", _.bind(function(socket, args) {
            var event = this.db.events.get(args.eventId);
            if(event && socket.user) {
                group.assignGroupToUser(socket.user.id, args.eventId).then(function(data) {
                    mgr.writeAck(socket, "group-user");
                    logger.info("assigned group " + data.sessionId);
                }).catch(function(err) {
                    logger.error("Error! " + err);
                    mgr.writeErr(socket, "group-user");
                });
            } else {
                mgr.writeErr(socket, "group-user");
            }
        ...
        this.db.events.on("connectedUsers:change:sessionPreference", function(event, user, sessionPreference) {            
            mgr.sync(event.getRoomId(), "state", {
                path: ["event", "connectedUsers", user.id, "sessionPreference"],
                op: "set",
