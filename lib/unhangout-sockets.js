...
_.extend(UnhangoutSocketManager.prototype, events.EventEmitter.prototype, {
    init: function(sockjs) {
        ...
        mgr.on("add-to-network", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);
            if (event && socket.user) {
                ...
                mgr.writeAck(socket, "add-to-network");
                event.logAnalytics({action: "add-to-network-sessions", user: socket.user});
            } else {
                mgr.writeErr(socket, "add-to-network");
            }
        }, this));
