    ...
    init: function(sockjs) {
        ...
        mgr.on("chat", _.bind(function(socket, args) {
            ...
            if (event && socket.user && mgr.roomContainsSocket(args.roomId, socket)) {
                ...
                // Handling atify for conversation network
                ...
                var matches = matchAll(/@([a-zA-Z0-9]+)/g, args.text);

                var selfName = normalize(socket.user.get("displayName"));
                ...
                _.each(matches, function(match) {
                    ...
                    if (selfName.indexOf(atname) == -1) {
                        ...
                        if (user) {
                            ...
                            //socket.user.save({networkList: []});

                            prevNetworkList = socket.user.get("networkList").otherUsers;
                            logger.debug("PREV LIST : " + prevNetworkList);

                            if(prevNetworkList != "" || prevNetworkList != undefined) {
                                //prevNetworkList = [];

                                if (prevNetworkList.indexOf(user.id) > -1) {
                                    return; 
                                }  
                            ...
                            prevNetworkList.push(user.id);
                            socket.user.save({networkList: {eventId: event.id, otherUsers: prevNetworkList}});
                            
                            logger.debug("NETWORK LIST : " + JSON.stringify(socket.user.get("networkList").otherUsers, 4, null));
        ...
        this.db.events.on("connectedUsers:change:networkList", function(event, user, networkList) {
            logger.debug("Network List Changed : " + networkList.eventId);
            
            // mgr.sync(networkList.eventId, "state", {
            //     path: ["user", "networkList"],
            //     op: "set", // note that "insert" will actually do a collection "add".
            //     value: networkList
            // });

            var key = ["event", networkList.eventId, "connectedUsers",
                       user.id, "networkList", "set"].join(":");

            mgr.throttledSync(key, networkList.eventId, "state", {
