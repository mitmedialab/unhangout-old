...
_.extend(UnhangoutSocketManager.prototype, events.EventEmitter.prototype, {
    init: function(sockjs) {
        ...
        mgr.on("vote-session", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);
            if (event && socket.user) {
                if (args.votes.indexOf(socket.user) < 0) {
                    args.votes.push(socket.user);
                    event.save({votes: args.votes});
                    mgr.writeAck(socket, "vote-session");
                    event.logAnalytics({action: "vote-session", user: socket.user});
                }
            } else {
        ...
        mgr.on("unvote-session", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);
            if (event && socket.user) {
                var indexOfUser = args.votes.indexOf(socket.user);
                if (indexOfUser > -1) {
                    args.votes.splice(indexOfUser, 1);
                    event.save({votes: args.votes});
                    mgr.writeAck(socket, "unvote-session");
                    event.logAnalytics({action: "unvote-session", user: socket.user});
                }
            } else {
                mgr.writeErr(socket, "unvote-session");
            }
        }, this));
        ...
        mgr.on("approve-session", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);
            if (event && socket.user && socket.user.isAdminOf(event)) {
                event.save({approved: args.approve});
        ...
        mgr.on("toggle-sessions-mode", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);
            if (event && socket.user && socket.user.isAdminOf(event)) {
                event.save({adminSessionsOnly: args.newMode});
                mgr.writeAck(socket, "toggle-sessions-mode");
                event.logAnalytics({action: "toggle-sessions-mode", user: socket.user});
            } else {
                mgr.writeErr(socket, "toggle-sessions-mode");
            }
        }, this));
        ...
        this.db.events.on("change:adminSessionsOnly", function(event, isAdminSessionsOnly) {
            mgr.sync(event.getRoomId(), "state", {
                path: ["event", "adminSessionsOnly"],
                op: "set",
