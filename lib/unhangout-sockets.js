    ...
    init: function(sockjs) {
        ...
        mgr.on("join", _.bind(function(socket, args) {
            ...
            if (channel == "event") {
                ...
                var participants = session.get("connectedParticipants");
                if (!_.any(participants, function(u) { return u.id == socket.user.id; })) {
                    participants.push(socket.user.toJSON());
                }
        ...
        mgr.on("leave", _.bind(function(socket, args) {
            ...
            if (channel == "event" && args.userLast) {
                ...
                participants = _.reject(participants, function(u) {
        ...
        mgr.on("session/set-hangout-url", _.bind(function(socket, args) {
            var session = this.getSession(args.sessionId);
            if (!session) {
