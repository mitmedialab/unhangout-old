    ...
    init: function(sockjs) {
        ...
        mgr.on("leave", _.bind(function(socket, args) {
            ...
            if (channel == "event" && args.userLast) {
                ...
                logger.info("user:" + (socket.user ? socket.user.id : "unknown") +
                            " leaving session:" + session.id);
                ...
                if (args.roomLast) {
                    ...
                    setTimeout(function() {
                        // A subtlety: if the session was an event session, but
                        // has been removed, we don't want to trigger 'save'
                        // for it and should no-op.
                        if (!session.get("isPermalinkSession") && !session.collection) {
                            return;
                        }
                        if (session.getNumConnectedParticipants() === 0) {
                            session.set("hangoutConnected", false);
                        }
                    }, models.ServerSession.prototype.HANGOUT_LEAVE_STOP_TIMEOUT);
                }
                session.save();
        ...
        mgr.on("session/set-hangout-broadcast-id", function(socket, args) {
            ...
            if (!session) {
                return mgr.writeErr(socket, route, "Session not found.");
            }
            ...
            session.set("hangout-broadcast-id", args["hangout-broadcast-id"]);
            session.save();
            ...
            // TODO: 
            // 1. Mute volume of event youtube video for people connected to the HoA.
