    ...
    init: function(sockjs) {
        ...
        mgr.on("vote-session", _.bind(function(socket, args) {
            ...
            if (event && socket.user) {
                var session = event.get("sessions").get(args.id);
                if (session) {
                    var vs = session.get("votes");
                    if (vs.indexOf(socket.user) < 0) {
                        vs.push(socket.user);
                        session.save({votes: vs});
                    }
        ...
        mgr.on("unvote-session", _.bind(function(socket, args) {
            ...
            if (event && socket.user) {
                ...
                if (session) {
                    var vs = session.get("votes");
                    var indexOfUser = vs.indexOf(socket.user);
                    if (indexOfUser > -1) {
                        vs.splice(indexOfUser, 1);
                        session.save({votes: vs});
                    }
                    mgr.writeAck(socket, "unvote-session");
                    event.logAnalytics({action: "unvote-session", user: socket.user, session: args.id});
                    return;
                }
            }
