                ...
                _.each(matches, function(match) {
                    ...
                    if (selfName.indexOf(atname) == -1) {
                        ...
                        if (user) {
                            ...
                            prevNetworkList = socket.user.get("networkList")
                            prevOtherUsers = prevNetworkList[event.id];

                            if(prevOtherUsers != "" || prevOtherUsers != undefined) {
                                prevOtherUsers = [];

                                if (prevOtherUsers.indexOf(user.id) > -1) {
                                    return; 
                                }  
                            ...
                            prevNetworkList[event.id] = prevOtherUsers;
                            socket.user.save({networkList: prevNetworkList});
                            logger.debug("NETWORK LIST : " + JSON.stringify(socket.user.get("networkList")[event.id], 4, null));
