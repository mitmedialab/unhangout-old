    ...
    init: function(sockjs) {
        ...
        mgr.on("join", _.bind(function(socket, args) {
            ...
            if (channel == "event") {
                ...
                var ids = session.get("connectedParticipantIds");
                if (!_.contains(ids, socket.user.id)) {
                    ids.push(socket.user.id);
                }
                session.setConnectedParticipantIds(ids);
                session.save();
            }
        }, this));
        ...
        mgr.on("leave", _.bind(function(socket, args) {
            ...
            if (channel == "event" && args.userLast) {
                ...
                var session = this.getSession(args.roomId);
                var ids = session.get("connectedParticipantIds")
                ids = _.without(ids, socket.user.id);
                session.setConnectedParticipantIds(ids);
                if (args.roomLast) {
