...
_.extend(UnhangoutSocketManager.prototype, events.EventEmitter.prototype, {
    init: function(sockjs) {
        ...
        mgr.on("participant-proposed", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);

            if (event && socket.user && socket.user.isAdminOf(event)) {
                event.save({adminProposedSessions: true});
                mgr.writeAck(socket, "participant-proposed");
                event.logAnalytics({action: "participant-proposed", user: socket.user});
            } else {
                mgr.writeErr(socket, "participant-proposed");
            }
        }, this));
        ...
        this.db.events.on("change:adminProposedSessions", function(event, isAdminSessionsOnly) {
            console.log("IS ADMIN SESSION ONLY : " + isAdminSessionsOnly);
