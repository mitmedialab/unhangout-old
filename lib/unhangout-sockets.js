...
_.extend(UnhangoutSocketManager.prototype, events.EventEmitter.prototype, {
    init: function(sockjs) {
        ...
        mgr.on("auto-close", _.bind(function(socket, args) {
            var event = this.getEvent(args.roomId);
            ...
            if (event && socket.user && socket.user.isAdminOf(event)) {      
                ...
                //iterate over the event users list and delete
                //their network list for a specific event 
                for(var i=0; i< eventUserIds.length; i++) {
                    var user = this.db.users.get(eventUserIds[i]); 
                    delete user.get("networkList")[event.id]; 
                }

                //delete the event history 
                event.save({history: {}}); 
                mgr.writeAck(socket, "auto-close");
                event.logAnalytics({action: "auto-close", user: socket.user});
            } else {
                mgr.writeErr(socket, "auto-close");
            }
        }, this));
