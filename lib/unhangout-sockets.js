    ...
    init: function(sockjs) {
        ...
        mgr.on("clear-previous-videos", function(socket, args) {
            ...
            if (event && socket.user.isAdminOf(event)) {
                ...
                mgr.roomToSockets[event.getRoomId()].forEach(function(s) {
                    if (s.user.isAdminOf(event)) {
                        mgr.writeData(s, "clear-previous-videos", {});
                    }
                });
            } else {
        ...
        this.db.events.on("sessions:change:connectedParticipants", function(event, session) {
            mgr.throttledBroadcast(event.getRoomId(), "session-participants", {
                 id: session.id,
                 participants: session.get("connectedParticipants")
            });
        });
        this.db.events.on("sessions:change:joiningParticipants", function(event, session) {
            mgr.throttledBroadcast(event.getRoomId(), "joining-participants", {
                id: session.id,
                participants: session.get("joiningParticipants")
            })
        });

        this.db.events.on("change:open", function(event, open) {
            mgr.broadcast(event.getRoomId(), "event-update", event.toClientJSON());
        });

        // Broadcast updates to hangouts-on-air.
        this.db.events.on("update-hoa", function(event, hoa) {
            if (hoa !== null) {
                // If it's the first time we've seen this broadcast ID, set it
                // as the embed.
                var youtubeId = hoa.get("hangout-broadcast-id");
                if (youtubeId !== null) {
                    ...
                    if (!visited) {
                        ...
                        event.setEmbed(hoa.get("hangout-broadcast-id"));
                        event.save();
                        mgr.broadcast(event.getRoomId(), "embed", {
                            ytId: hoa.get("hangout-broadcast-id"), roomId: event.getRoomId()
                        });
                    }
                }
            }
            ...
            _.each(mgr.roomToSockets[event.getRoomId()], function(s) {
                if (s.user.isAdminOf(event)) {
                    var data = hoa === null ? null : hoa.toClientJSONForUser(s.user);
                    mgr.writeData(s, "set-hoa", data);
                }
            });
        });
    },
