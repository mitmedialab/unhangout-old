...
exports.init = function(app) {
	...
	app.express.get('/hangout-callback', function(req, res) {
		...
		oauth2Client.getToken(req.query["code"], function(err, token) {
				...
				.insert({
					...
					.execute(function(err, event) {
						...
						if(!err && "hangoutLink" in event) {
							...
							app.redis.lpush("global:hangout_urls", event.hangoutLink, function(err, res) {
								app.logger.info("err: " + err);
								app.logger.info("logged new hangout url: " + event.hangoutLink + "; total: " + res);
							});
						}
					});
				});
			...
			res.send("<a href='http://" + app.options.HOST + ":" + app.options.PORT + "/hangout-farming'>CLICK ME</a>");
		});
	});
