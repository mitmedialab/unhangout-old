var events = require('events'),
    ...
    monotonic = require("./monotonic-counter"),
...
_.extend(exports.RoomManager.prototype, events.EventEmitter.prototype, {
    ...
    joinWithoutAuth: function (socket, roomId, timestamp) {
        ...
        if (timestamp) {
            ...
            var hrnow = monotonic.timestamp(); // [milliseconds, counter]
    ...
    writeData: function(socket, type, args, timestamp) {
        timestamp = timestamp || monotonic.timestamp();
    ...
    broadcast: function(roomId, type, args, filter, timestamp) {
        var timestamp = timestamp || monotonic.timestamp();
        var sockets = this.roomToSockets[roomId];
    ...
    sync: function(roomId, type, args, filter) {
        ...
        // prune old logs.
        // logs look like: [
        //   [[milliseconds, counter], <type>, <args>, <filter()>],
        //   ...
        // ]
        var cutoff = hrnow[0] - this.OP_LOG_AGE;
    ...
    throttledSync: function(key, roomId, type, args) {
