...
* first "/" delimiter -- e.g. the room named "admins/chat" has the channel
...
*   rm.channelAuth.admins = function(user, room, callback) {
*       var authorized = user.get("admin") === true;
...
exports.RoomManager = function (socketServer, users) {
    ...
    // take the signature:  function(user, room, callback), and should check
...
_.extend(exports.RoomManager.prototype, events.EventEmitter.prototype, {
    ...
    destroy: function() {
        ...
        for (var socketId in this.socketBindings) {
            ...
            logger.debug("unbinding ID " + socketId);
        }
    },
    handleConnection: function(socket, data) {
        ...
        bindings.events.data = _.bind(function(message) {
            logger.debug("RoomManager message", message);
            var data;
            try {
        ...
        logger.debug("binding ID" + socket.id);
    ...
    route: function (socket, data) {
        if (data.type === "auth") {
            ...
            if (data.args && "id" in data.args) {
                this.handleJoinRequest(socket, data.args.id);
            } else {
    ...
    handleAuth: function (socket, args) {
        ...
        logger.info("AUTHENTICATED sock " + socket.id + " to user " + user.id);
        socket.user = user;
        this.socketIdToUser[socket.id] = user;
        var sockets = this.userIdToSockets[user.id] || [];
        if (!_.findWhere(sockets, function(s) { return s.id == socket.id })) {
            sockets.push(socket);
        }
    ...
    handleLeave: function(socket, roomId) {
        ...
        if (rooms) {
            ...
            if (rooms.length == 0) {
                ...
                this.socketIToRooms[socket.id] = rooms;
            }
        }
        // if there are no sockets, or none of the sockets belong to our user,
        // we are the last user.
        userLast = !sockets || !_.any(sockets, function(s) { return s.user.id == socket.user.id });
        if (users && userLast) {
    ...
    writeUser: function(user, data) {
        // Call "writeData" for each socket belonging to this user.
        var sockets = this.userIdToSockets[user.id] || [];
        _.each(sockets, _.bind(function(s) { this.writeData(s, data); }, this));
    },
    writeAck: function(socket, type, args) {
        this.writeData(socket, {type: type + "-ack", args: args});
    },
    writeErr: function(socket, type, args) {
        logger.warn("Socket error", {type: type, args: args});
        this.writeData(socket, {type: type + "-err", args: args});
    },
    ...
    broadcast: function(roomId, data, exclude) {
        // Call "writeData" for each socket that has joined the given room,
        // optionally excluding the socket passed as "exclude".
        var sockets = this.roomToSockets[roomId];
        _.each(sockets, _.bind(function(s) {
            if (exclude && s.id == exclude.id) {
                return;
            }
            this.writeData(s, data);
        }, this));
    }
});
