...
exports.init = function(logger, redis) {
	redis = redis;
	logger = logger;
...
exports.sync = function(method, model, options) {
	if(!redis || !logger) {
		console.log("Used sync before intializing it with a redis and logger connection!");
		return;
	}
	
	if(!redis.connected) {
		logger.fatal("tried to sync, but redis is not connected");
		return;
	}
