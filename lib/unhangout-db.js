    ...
    loadModels: function(callback) {
        ...
        var loaders = [
            ...
            ["event/*/sessions/*", function(callback, attrs, key) {
                ...
                // Reset connected participants when loading from redis.  We
                // should only be here if the server was reset; and sockets
                // will have to re-connect. (NOTE: this is duplicated below)
                var numConnected = newSession.getNumConnectedParticipants();
                if (numConnected > 0) {
                    logger.warn("Orphaned session users in session " + newSession.id + ": " + numConnected);
                    newSession.set("connectedParticipants", []);
                }
            ...
            ["session/permalink/*", function(callback, attrs, key) {
                ...
                // Reset connected participants when loading from redis.  We
                // should only be here if the server was reset; and sockets
                // will have to re-connect. (NOTE: this is duplicated above)
                var numConnected = newSession.getNumConnectedParticipants();
                if (numConnected > 0) {
                    logger.warn("Orphaned session users in session " + newSession.id + ": " + numConnected);
                    newSession.set("connectedParticipants", []);
