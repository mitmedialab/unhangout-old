...
module.exports = function(db, options) {
    ...
    group.assignRandomizedSession = function(eventId, userId) {
        ...
        return Promise.resolve().then(function() {
            ...
            if (pref && pref[eventId] && pref[eventId].length > 0) {
                // remove user as assigned participant from each session.
                pref[eventId].forEach(function(sessionId) {
                    var session = db.events.getSessionById(sessionId);
                    if (session) {
                ...
                randomizedSessionsList = _.filter(randomizedSessionsList, function(sess) {
                    return !_.contains(pref[eventId], sess.id);
                });
