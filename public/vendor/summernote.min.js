! function (a) {
    ...
    var c = {
                ...
                return a.Deferred(function (c) {
                    ...
                    return a.Deferred(function (c) {
                        a.extend(new Image, {
                            ...
                            onload: function () {
                                this.detachEvents(), c.resolve(this)
                            },
                            onerror: function () {
                                this.detachEvents(), c.reject(this)
                            },
                            onabort: function () {
                                this.detachEvents(), c.reject(this)
                            }
        ...
        i = function () {
            ...
            }, j = ["bold", "italic", "underline", "strikethrough", "justifyLeft", "justifyCenter", "justifyRight", "justifyFull", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "formatBlock", "removeFormat", "backColor", "foreColor", "insertHorizontalRule"], l = 0, m = j.length; m > l; l++) this[j[l]] = function (a) {
                return function (b, c) {
            ...
            this.tab = function (b) {
                e(b);
                var c = g.create(),
                    d = new Array(b.data("tabsize") + 1).join("&nbsp;");
                c.insertNode(a('<span id="noteTab">' + d + "</span>")[0]);
                ...
                h.loadImage(c).done(function (d) {
                    ...
                    f.css("width", Math.min(b.width(), d.width)), g.create().insertNode(f[0])
                }).fail(function () {
            ...
            }, this.setLinkDialog = function (b, d) {
                var h = g.create();
                if (h.isOnAnchor()) {
                ...
                d({
                    ...
                    h.select(), e(b);
                    var f;
                    if (f = -1 !== d.indexOf("@") ? -1 !== d.indexOf(":") ? d : "mailto:" + d : -1 !== d.indexOf("://") ? d : "http://" + d, c.bMSIE && h.isCollapsed()) {
                ...
                b({
                    ...
                    c.select(), e(a), d.insertVideo(a, b)
                })
                ...
                e(a);
                var c = b.split("x");
                g.create().insertNode(d.createTable(c[0], c[1]))
            }, this.floatMe = function (a, b, c) {
                e(a), c.style.cssFloat = b
            }, this.resize = function (a, b, c) {
                e(a), c.style.width = a.width() * b + "px", c.style.height = ""
            ...
            this.showImageDialog = function (c, d, e) {
                var f = c.find(".note-image-dialog"),
                    g = c.find(".note-image-input"),
                    h = c.find(".note-image-url"),
                    i = c.find(".note-image-btn");
                ...
                var e = a.find(".note-video-dialog"),
                    f = e.find(".note-video-url"),
                    g = e.find(".note-video-btn");
                e.on("shown.bs.modal", function () {
                    ...
                    f.off("keyup"), g.off("click"), e.off("shown.bs.modal hidden.bs.modal")
                }).modal("show")
            }, this.showLinkDialog = function (a, c, d) {
                var e = a.find(".note-link-dialog"),
                    f = e.find(".note-link-text"),
                    g = e.find(".note-link-url"),
                    h = e.find(".note-link-btn");
                e.on("shown.bs.modal", function () {
            ...
            var d = new l,
                ...
                k = {
                    ...
                    var b = c.bMac ? a.metaKey : a.ctrlKey,
                        ...
                        g = b || e || f === k.TAB,
                        h = g ? q(a.target) : null;
                    if (f === k.TAB && h.editable().data("tabsize")) d.tab(h.editable());
                    else if (b && (e && f === k.Z || f === k.Y)) d.redo(h.editable());
                    else if (b && f === k.Z) d.undo(h.editable());
                    else if (b && f === k.B) d.bold(h.editable());
                    else if (b && f === k.I) d.italic(h.editable());
                    else if (b && f === k.U) d.underline(h.editable());
                    else if (b && e && f === k.S) d.strikethrough(h.editable());
                    else if (b && f === k.BACKSLACH) d.removeFormat(h.editable());
                    else if (b && f === k.K) h.editable(), d.setLinkDialog(h.editable(), function (a, b) {
                        j.showLinkDialog(h.dialog(), a, b)
                    });
                    else if (b && f === k.SLASH) j.showHelpDialog(h.dialog());
                    else if (b && e && f === k.L) d.justifyLeft(h.editable());
                    else if (b && e && f === k.E) d.justifyCenter(h.editable());
                    else if (b && e && f === k.R) d.justifyRight(h.editable());
                    else if (b && e && f === k.J) d.justifyFull(h.editable());
                    else if (b && e && f === k.NUM7) d.insertUnorderedList(h.editable());
                    else if (b && e && f === k.NUM8) d.insertOrderedList(h.editable());
                    else if (b && f === k.LEFTBRACKET) d.outdent(h.editable());
                    else if (b && f === k.RIGHTBRACKET) d.indent(h.editable());
                    else if (b && f === k.NUM0) d.formatBlock(h.editable(), "P");
                    else if (b && k.NUM1 <= f && f <= k.NUM6) {
                        var i = "H" + String.fromCharCode(f);
                        d.formatBlock(h.editable(), i)
                    } else {
                        if (!b || f !== k.ENTER) return (f === k.BACKSPACE || f === k.ENTER || f === k.SPACE) && d.recordUndo(q(a.target).editable()), void 0;
                    ...
                    if (h.length > 0) {
                        ...
                        if (d[k] && (r.trigger("focus"), d[k](r, l, i)), -1 !== a.inArray(k, ["backColor", "foreColor"])) e.updateRecentColor(h[0], k, l);
                        ...
                        else if ("showImageDialog" === k) r.focus(), j.showImageDialog(p, function (a) {
                            s(r, a)
                        }, function (a) {
                            ...
                            if (y) r.data("orgHeight", r.css("height")), a(window).resize(x).trigger("resize");
                            else {
                            ...
                            if (A) {
                                if (t.val(r.html()), t.height(r.height()), e.disable(o), t.focus(), c.bCodeMirror) {
                                    var B = b.fromTextArea(t[0], a.extend({
                                        mode: "text/html",
                                        lineNumbers: !0
                                    }, n.data("options").codemirror));
                                    B.setSize(null, r.outerHeight()), B.autoFormatRange && B.autoFormatRange({
                                        line: 0,
                                        ch: 0
                                    }, {
                                        line: B.lineCount(),
                            ...
                            } else c.bCodeMirror && t.data("cmEditor").toTextArea(), r.html(t.val() || f.emptyPara), r.height(r.data("optionHeight") ? t.height() : "auto"), e.enable(o), r.focus();
            ...
            b = {
                ...
                bold: function (a) {
                    return '<button type="button" class="btn btn-default btn-sm btn-small" title="' + a.font.bold + '" data-event="bold" tabindex="-1"><i class="fa fa-bold icon-bold"></i></button>'
                },
                italic: function (a) {
                    return '<button type="button" class="btn btn-default btn-sm btn-small" title="' + a.font.italic + '" data-event="italic" tabindex="-1"><i class="fa fa-italic icon-italic"></i></button>'
                },
                underline: function (a) {
                    return '<button type="button" class="btn btn-default btn-sm btn-small" title="' + a.font.underline + '" data-event="underline" tabindex="-1"><i class="fa fa-underline icon-underline"></i></button>'
                },
                clear: function (a) {
                    return ''
                },
                ul: function (a) {
                    return '<button type="button" class="btn btn-default btn-sm btn-small" title="' + a.lists.unordered + '" data-event="insertUnorderedList" tabindex="-1"><i class="fa fa-list-ul icon-list-ul"></i></button>'
                },
                ol: function (a) {
                    return '<button type="button" class="btn btn-default btn-sm btn-small" title="' + a.lists.ordered + '" data-event="insertOrderedList" tabindex="-1"><i class="fa fa-list-ol icon-list-ol"></i></button>'
                },
                paragraph: function (a) {
                    return '<button type="button" class="btn btn-default btn-sm btn-small dropdown-toggle" title="' + a.paragraph.paragraph + '" data-toggle="dropdown" tabindex="-1"><i class="fa fa-align-left icon-align-left"></i>  <span class="caret"></span></button><ul class="dropdown-menu"><li><div class="note-align btn-group"><button type="button" class="btn btn-default btn-sm btn-small" title="' + a.paragraph.left + '" data-event="justifyLeft" tabindex="-1"><i class="fa fa-align-left icon-align-left"></i></button><button type="button" class="btn btn-default btn-sm btn-small" title="' + a.paragraph.center + '" data-event="justifyCenter" tabindex="-1"><i class="fa fa-align-center icon-align-center"></i></button><button type="button" class="btn btn-default btn-sm btn-small" title="' + a.paragraph.right + '" data-event="justifyRight" tabindex="-1"><i class="fa fa-align-right icon-align-right"></i></button><button type="button" class="btn btn-default btn-sm btn-small" title="' + a.paragraph.justify + '" data-event="justifyFull" tabindex="-1"><i class="fa fa-align-justify icon-align-justify"></i></button></div></li><li><div class="note-list btn-group"><button type="button" class="btn btn-default btn-sm btn-small" title="' + a.paragraph.outdent + '" data-event="outdent" tabindex="-1"><i class="fa fa-outdent icon-indent-left"></i></button><button type="button" class="btn btn-default btn-sm btn-small" title="' + a.paragraph.indent + '" data-event="indent" tabindex="-1"><i class="fa fa-indent icon-indent-right"></i></button></li></ul>'
                },
                height: function (a) {
                    return ''
                },
            ...
            this.createLayout = function (c, i) {
                var k = i.height,
                    ...
                    m = i.direction,
                    n = i.toolbar,
                    o = a.summernote.lang[i.lang],
                    q = c.next();
                if (q || !q.hasClass("note-editor")) {
                    var s = a('<div class="note-editor"></div>');
                    s.data("options", i), k > 0 && a('<div class="note-statusbar">' + h + "</div>").prependTo(s);
                    var t = a('<div class="note-editable" contentEditable="true"></div>').prependTo(s);
                    k && (t.height(k), t.data("optionHeight", k)), l && t.data("tabsize", l), m && t.attr("dir", m), t.html(f.html(c) || f.emptyPara), t.data("NoteHistory", new j), a('<textarea class="note-codable"></textarea>').prependTo(s), setTimeout(function () {
                        document.execCommand("styleWithCSS", 0, !0)
                    });
                    for (var u = "", v = 0, w = n.length; w > v; v++) {
                        var x = n[v];
                        u += '<div class="note-' + x[0] + ' btn-group">';
                        for (var y = 0, z = x[1].length; z > y; y++) u += b[x[1][y]](o);
                        u += "</div>"
                    }
                    u = '<div class="note-toolbar btn-toolbar">' + u + "</div>";
                    var A = a(u).prependTo(s);
                    r(A), p(A, "bottom");
                    var B = a(d(o)).prependTo(s);
                    p(B), a(e).prependTo(s);
                    var C = a(g(o)).prependTo(s);
                    C.find("button.close, a.modal-close").click(function () {
                        a(this).closest(".modal").modal("hide")
                    }), a('<div class="note-dropzone"><div class="note-dropzone-message"></div></div>').prependTo(s), s.insertAfter(c), c.hide()
                }
            };
        ...
        t = new q;
    a.summernote = a.summernote || {}, a.extend(a.summernote, {
        version: "0.5.1",
        lang: {
            "en-US": {
                font: {
